...
export function isRandomEvent(chance, base) {
    const r = randomInt(0, base);
    return (r < chance);
}
...
// TODO: all operations with vectors must return new instances, don't change input!
...
export function dotProduct3d(v1, v2) {
    // Скалярное произведение 3д векторов - dot product
...
export function murmurHash(input) {
    // Usage:
    // const input = [42, 3.14, 10, 7.5];
    // const hash = murmurHash(input);
    // console.log('MurmurHash:', hash);
    let hash = 0;  
    for (let i = 0; i < input.length; i++) {
        const value = input[i];
        const bytes = new Uint8Array(Float64Array.of(value).buffer);
  
        let h = 0x31415926; // Seed value
        for (let j = 0; j < bytes.length; j++) {
            h ^= bytes[j];
            h = (h * 0x01000193) >>> 0; // Prime number
        }
        hash = (hash + h) >>> 0; // Ensure hash is a 32-bit unsigned integer
    }
    return hash;
}